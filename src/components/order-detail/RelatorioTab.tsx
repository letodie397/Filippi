...
import { Plus, Trash2, Save, ImagePlus, X, FileDown, FileText } from 'lucide-react'
...
export function RelatorioTab({ order, relatorios: initialRelatorios, materiais, saving, onSave }: RelatorioTabProps) {
  ...
  function addImagem(id: string, file: File) {
    const reader = new FileReader()
    reader.onload = (ev) => {
      const dataUrl = ev.target?.result as string
      setEntries((prev) =>
        prev.map((e) => (e.id === id ? { ...e, imagens: [...e.imagens, dataUrl] } : e))
      )
      setDirty(true)
    }
              ...
              <div className="px-5 py-4 space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                    ...
                    {/* Add image button */}
                    <label className="flex flex-col items-center justify-center h-24 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-icm-red-400 hover:bg-red-50/30 transition-colors">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          files.forEach((f) => addImagem(entry.id, f))
                          e.target.value = ''
                        }}
                      />
                    </label>
